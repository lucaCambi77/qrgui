...
public class QueryExecutor {
    ...
    public WrappedResponse<QueryExecution> call(
            final UteQueDto query, QueryToJson json, Integer page, Integer pageSize, QueryType queryType) {
        ...
        if (queryType == QueryType.COUNT) {
            execution = new QueryExecutionCountResponse(firstGenericDao.executeQueryCount(queryStringResponse.getEntity()).intValue(), query);
        } else {
            execution = new QueryExecutionListResponse(queryService.parseResultSet(json, firstGenericDao.getByNativeQuery(queryStringResponse.getEntity(), page, pageSize)), query);
        }
