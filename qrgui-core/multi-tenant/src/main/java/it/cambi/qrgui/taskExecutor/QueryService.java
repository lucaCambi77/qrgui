...
public class QueryService {
  ...
  public WrappedResponse<QueryToJson> checkQuery(
      QueryToJson json, Optional<BiFunction<String, Integer, List<Object>>> queryCheckFunction) {
      ...
      switch (attr.getParameter().getType()) {
        ...
        case NUMBER -> {
          if (attr.getOperator()
            ...
            String[] split = cleanQueryString(attr.getParameter().getValue()).split(",");
          ...
          if (attr.getOperator()
            ...
            finaleReplace =
                finaleReplace.replace(
                    attr.getParameter().getName(),
                    "("
                        + Arrays.stream(cleanQueryString(attr.getParameter().getValue()).split(","))
                            .map(s -> "'" + s + "'")
