...
angular.module('qrGuiApp').factory('HtmlUtilityFactory', ['$compile', '$rootScope', 'DateUtilityFactory', 'constant',
    ...
    function HtmlUtilityFactory($compile, $rootScope, DateUtilityFactory, constant) {
        ...
        function SetRoutineListTemplate(scope, routines) {
            ...
            const factor = (null != $rootScope.ertaQrGuiUser && $rootScope.ertaQrGuiUser.admin) ? 3
                : 4;
             ...
             * bootstrap x12
             */
            const rowDim = 12 / factor;

            const routineLenght = routines.length;

            const parts = routineLenght / factor;
            let start = 0, templateRoutine = "";

            const rows = Math.floor(parts);
        ...
        function SetCategoriesListTemplate(scope, categories) {
            ...
            const categoryLenght = $rootScope.categories.length;

            const parts = categoryLenght / factor;
            let start = 0, template = "";

            const rows = Math.floor(parts) + 1;
        ...
        function GetQueryListTemplate(resultList, attrlist, queryPosition) {
            ...
            const template = GetQueryTableTemplate(resultList, attrlist);

            /* Paginatore */
            const pagination = GetQueryPaginationTemplate(queryPosition);
        ...
        function GetQueryTableTemplate(resultList, attrlist) {
            ...
            let i;
            let template = '<div class="panel panel-default" style="margin: 1em;"> <div class="table-responsive"> <table class="table table-responsive table-bordered table-striped table-condensed table-bordered">';

            let firstRow = '<thead> <tr>';
