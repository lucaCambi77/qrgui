...
angular
  .module('qrGuiApp')
  .directive(
    'routineCategories', ['$compile', '$rootScope',
    function ($compile, $rootScope) {
      return {
         ...
         * aggiungi
         */
          const isAdmin = $rootScope.ertaQrGuiUser != null &&
            $rootScope.ertaQrGuiUser.admin
          const categoryLength = ($rootScope.categories == null)
            ? 0
            : $rootScope.categories.length

          const length = isAdmin
            ? (categoryLength - 1)
            : categoryLength

          const subCateg = '<div ng-repeat="item in cat.childrens">' +
            '	      <div style="margin : 1em;" class="panel panel-default">' +
            '			<div class="panel-heading"> <h4 class="panel-title">' +
            '     	 <a data-toggle="collapse" data-parent="#accordion" data-target="#collapse-{{item.root.cat}}"' +
          ...
          const templ = '<div ng-repeat="cat in categories | limitTo:' +
            length.toString() +
            '"> <div style="margin : 1em;" class="panel panel-default">' +
            '  <div class="panel-heading">' +
            '    <h4 class="panel-title">' +
            '      <a data-toggle="collapse" data-parent="#accordion" data-target="#collapse-{{cat.root.cat}}"' +
            ' 	href><i class="fa fa-book" aria-hidden="true"></i>' +
            ...
            ctrl.$parsers
              .push(function (value) {
                ...
                if (Object.prototype.toString
                  .call(value) === '[object Date]') { return value.getTime() }
