...
public class Temi15Service implements ITemi15Service<Temi15UteQue>
{
    ...
    @Autowired
    private ITemi18Dao<Temi18RouQue, Temi18RouQueId> queRouAssDao;
    ...
    public WrappedResponse<Temi15UteQue> getByPk(Long cQue, Long dateIns)
    {
        ...
        return new WrappedResponse<Temi15UteQue>().setEntity(query == null ? new Temi15UteQue() : query).setCount(query == null ? 0 : 1)
                .setResponse();
    ...
    public WrappedResponse<Temi15UteQue> deleteQuery(Temi15UteQueId cque)
    {
        ...
        return new WrappedResponse<Temi15UteQue>().setEntity(queryDao.delete(queryDao.getEntityByPrimaryKey(cque))).setResponse();
    }
    ...
    public List<Temi16QueCatAss> getQueCatByQueryId(Temi15UteQueId cque)
    {
        ...
        ParameterExpression<Long> cQueParam = queCatAssDao.getEntityManager().getCriteriaBuilder()
        ...
        List<Temi16QueCatAss> temi16List = queCatAssDao.getEntityManager().createQuery(criteriaQuery.where(predicateCQue, predicateIns))
    ...
    public WrappedResponse<List<Temi15UteQue>> getByTipCateg(List<String> listAllowedCat, List<Temi15UteQue> queriesIn, HttpServletRequest request)
    {
        ...
        criteriaQueryPar.multiselect(rootPar.get("id").get("que"), rootPar.get("id").get("insQue"),
        ...
        criteriaQueryPar.having(queCatAssDao.getEntityManager().getCriteriaBuilder()
                .gt(queCatAssDao.getEntityManager().getCriteriaBuilder().count(rootPar.get("id").get("que")), 0));
        ...
        if (null != listTemi15 && listTemi15.size() == 0)
            return new WrappedResponse<List<Temi15UteQue>>().setEntity(new ArrayList<>()).setCount(0)
                    .setResponse();
        ...
        {
            {
                listTemi15.forEach((query) -> {
                    ...
                    add(queryDao.getEntityManager().getCriteriaBuilder().and(predicateQue, predicateinsQue));
        ...
        return new WrappedResponse<List<Temi15UteQue>>().setEntity(listTemi15Out).setCount(listTemi15Out.size())
                .setResponse();
    }
