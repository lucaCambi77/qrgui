...
public class GenericQueryTaskExecutorService
{
    ...
    /**
     * Metodo per l'esecuzione simultanea di tutte le query. Contiene un executor service che lancia N callable. Per l' {@link ExecuteQueryTask} ,
     * vedi applicationContext.xml, in cui viene definita la factory {@link QueryExecutorFactoryImpl} che col metodo getExecuteQueryTask restitiusce
            ...
            throws InterruptedException, ExecutionException, JsonParseException, JsonMappingException, IOException
    {
        ...
        List<Future<XWrappedResponse<Temi15UteQue, List<Object>>>> taskList = new ArrayList<Future<XWrappedResponse<Temi15UteQue, List<Object>>>>();
        ...
        for (Temi15UteQue query : queryAttributes)
        {
            ...
            FutureTask<XWrappedResponse<Temi15UteQue, List<Object>>> myTask = new FutureTask<XWrappedResponse<Temi15UteQue, List<Object>>>(
                    queryExecutorFactory.getExecuteQueryTask().setPage(page).setPageSize(pageSize).setQuery(aCopy));
             ...
             * Task Executor
             */
            FutureTask<XWrappedResponse<Temi15UteQue, List<Object>>> myTaskCount = new FutureTask<XWrappedResponse<Temi15UteQue, List<Object>>>(
                    queryExecutorFactory.getExecuteQueryTask().setPage(page).setPageSize(pageSize).setQuery(anotherCopy));
        ...
        for (XWrappedResponse<Temi15UteQue, List<Object>> response : listOut)
        {
            ...
            if (json.getPosition() - position == 0 && json.getQueryType() == QueryType.COUNT)
            {
                ...
                {
                    QueryToJson json1 = objectMapper.readValue(response1.getXentity().getJson(), QueryToJson.class);
                    if (json1.getPosition() - position == 0
                            && json1.getQueryType() == QueryType.RESULT_SET)
                    {
